    ...
    (function() {
        ...
        function run() {
            ...
            loadMeta().then(function(meta) {
                ...
                shown = 0;
                // Normalise the meter to the displayed score range so the bars
                // actually show variance instead of all sitting at the same pct.
                if (currentResults.length) {
                    var lo = currentResults[currentResults.length - 1][0];
                    var hi = currentResults[0][0];
                    scoreRange = [lo, Math.max(hi, lo + 1)];
                }
